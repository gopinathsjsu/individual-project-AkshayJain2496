...
public class ReadCsv {
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        Set<String> MiscSeen = new HashSet<>();
        for(String[] map : inputs) {
            cards.add(map[2]);

            double amount= essentials.calculate(inventoryItems,map[0],Integer.parseInt(map[1]),corrections,essentialsSeen,luxurySeen,MiscSeen);
            totalAmount+=amount;
        }
        if(EssentialLimit.getTotalEssential()<0)
            corrections.addAll(essentialsSeen);

        if(EssentialLimit.getTotalLuxury()<0)
            corrections.addAll(luxurySeen);

        if(EssentialLimit.getTotalMisc()<0)
            corrections.addAll(MiscSeen);
        //Create record
        ...
        if(corrections.size()==0) {
            ...
            writting.add(new String[]{"Amount Paid"});
